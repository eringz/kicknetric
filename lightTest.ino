const int togglePin = 2;       // Pin connected to the SPDT toggle switch
const int togglePin2 = 4;       // Pin connected to the SPDT toggle switch
...
void setup() {
  // 
  pinMode(togglePin, INPUT_PULLUP); // Set the toggle pin as input with internal pull-up resistor
  for (int i = 0; i <= 12; i++) {
...
void loop() {
  Get_Lidar_data();
  //
  int toggleState = digitalRead(togglePin); // Read the state of the toggle switch
  int toggleState2 = digitalRead(togglePin2); // Read the state of the toggle switch

  // If the toggle switch is in the left position
  if (toggleState == HIGH) {
  ...
  // If the toggle switch is in the right position
